  ...
  return (
    <Grid container spacing={3}>
      ...
      <Grid item xs={12} md={8}>
        ...
        >
          {tour ? tour?.map((tour) => <ShopProductCard tour={tour} />) : <></>}
        </Box>
      </Grid>
    </Grid>
  );
    ...
    <Card sx={{ borderRadius: 2, bgcolor: 'background.neutral' }}>
      ...
      <Stack spacing={2.5} sx={{ p: 3, pb: 2.5 }}>
        <Stack direction="row" alignItems="center" spacing={1}>
          <div>
            <Link
              to={localStorage.getItem('accessToken') ? linkTo : PATH_PAGE.details}
              color="inherit"
              onClick={() => localStorage.setItem('tourId', tour?._id)}
      ...
      <InfoStyle>
        ...
        >
          <Typography variant="h4">
            {myProfile[0]?.fname} {myProfile[0]?.lname}
          </Typography>
          {/* <Typography sx={{ opacity: 0.72 }}>{position}</Typography> */}
          <Rating name="read-only" value={myProfile ? myProfile[0]?.rating : 0} readOnly />
        </Box>
      </InfoStyle>
      <Image
        alt="profile cover"
        src={myProfile[0]?.profilePicture}
